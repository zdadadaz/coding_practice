...
class Solution:
    def largestDivisibleSubset(self, nums):
        out = self.recur([-1]*len(nums),nums,{})
        ans = [nums[idx] for idx in range(len(nums)) if out[1][idx] ==1]
        return ans
    def check(self,s,nums,newidx):
        if 1 not in s:
            return True
        for i in range(len(s)):
            if s[i]==1:
                if (nums[newidx] % nums[i]!=0) and (nums[i]%nums[newidx]!=0):
                    return False
        return True
    def recur(self,s, nums,_V):
        if -1 not in s:
            return (1,tuple(s))
        else:
            if tuple(s) not in _V:
                arr=[]
                for idx,a in enumerate(nums):
                    if s[idx]==-1:
                        if self.check(s, nums,idx):
                            stmp = copy.deepcopy(s)
                            stmp[idx] = 1
                            # print("stmp",stmp)
                            re = self.recur(stmp, nums,_V)
                            # print(re[0],re[1])
                            arr.append((re[0]+1,re[1]))
                        else:
                            stmp = copy.deepcopy(s)
                            stmp[idx] = 0
                            re = self.recur(stmp, nums,_V)
                            arr.append((re[0]+0,re[1]))
                _V[tuple(s)] = max(arr)
            return _V[tuple(s)]
    # O(n^2), 
    def largestDivisibleSubset_web(self, nums):
        if len(nums)==0:
            return []
        nums.sort()
        countdiv = [1]*len(nums)
